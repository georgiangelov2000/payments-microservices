...
class AnalyticsRepository
{
    ...
    public function getOverview(string $merchantId, int $days = 30, string $environment = 'test'): array
    {
        ...
        $avgLatency = DB::table('payment_routing_attempts')
            ->join('payments', 'payments.id', '=', 'payment_routing_attempts.payment_id')
            ->where('payments.merchant_id', $merchantId)
            ->where('payments.environment', $environment)
            ->where('payments.created_at', '>=', $since)
            ->whereNotNull('payment_routing_attempts.latency_ms')
            ->avg('payment_routing_attempts.latency_ms');
     ...
     */
    public function getDailyTrend(string $merchantId, int $days = 30, string $environment = 'test', ?string $status = null): array
    {
        $since = Carbon::now()->subDays($days)->startOfDay();
    ...
    /**
     * Provider-level payment status counts and successful-payment volume.
     */
    public function getProviderPerformance(string $merchantId, int $days = 30, string $environment = 'test'): array
    {
        ...
        $rows = DB::table('payments')
            ...
            ->selectRaw('
                ...
                SUM(CASE WHEN payments.status = ? THEN 1 ELSE 0 END) AS succeeded,
                SUM(CASE WHEN payments.status IN (?, ?) THEN 1 ELSE 0 END) AS pending,
                SUM(CASE WHEN payments.status = ? THEN 1 ELSE 0 END) AS failed,
                COALESCE(SUM(CASE WHEN payments.status = ? THEN payments.price ELSE 0 END), 0) AS paid_volume,
                COALESCE(AVG(CASE WHEN payments.status = ? THEN payments.price END), 0) AS avg_payment,
                COALESCE(MIN(COALESCE(payments.currency, \'USD\')), \'USD\') AS currency,
                COUNT(DISTINCT COALESCE(payments.currency, \'USD\')) AS currencies_count
            ', [
                PaymentStatus::FINISHED->value,
        ...
        return $rows->map(fn ($r) => [
            ...
            'failed' => (int) $r->failed,
            'paid_volume' => (float) $r->paid_volume,
            'avg_payment' => (float) $r->avg_payment,
            'currency' => $r->currency ?? 'USD',
            'currencies_count' => (int) $r->currencies_count,
        ])->toArray();
    }
    ...
    public function getLatencyBuckets(string $merchantId, int $days = 30, string $environment = 'test'): array
    {
        ...
        foreach ($buckets as $label => [$min, $max]) {
            $query = DB::table('payment_routing_attempts')
                ...
                ->where('payments.created_at', '>=', $since)
                ->whereNotNull('payment_routing_attempts.latency_ms')
                ->where('payment_routing_attempts.latency_ms', '>=', $min);

            if ($max !== PHP_INT_MAX) {
                $query->where('payment_routing_attempts.latency_ms', '<', $max);
            }
